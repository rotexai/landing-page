...
const PricingCard = ({ name, tagline, priceLabel, features, ctaLabel, ctaHref, featured, style }) => {
    return (
        <div
            data-reveal
            style={style}
        ...
        >
            {featured && (
                <div className="absolute top-0 right-0 z-20 border-l border-b border-white/90 bg-white/[0.08] text-zinc-900 text-xs font-bold px-4 py-1 rounded-bl-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.95)] backdrop-blur-2xl">
                    Most Popular
                </div>
            )}
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
                <div data-reveal className="mb-16 text-center">
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold text-zinc-900 mb-4 font-['Poppins']">
                        Pricing For Every Stage Of <span className="text-amber-500">Human-AI Adoption</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
                    {pricing.map((tier, index) => (
                        <PricingCard key={tier.id} {...tier} style={{ '--reveal-delay': `${index * 110}ms` }} />
                    ))}
                </div>
            </div>
        </section>
